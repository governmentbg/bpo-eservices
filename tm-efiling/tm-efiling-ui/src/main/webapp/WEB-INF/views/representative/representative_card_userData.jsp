...
<td data-val="id"><c:if test="${param.representativeIsImported}">${param.representativeId}</c:if></td>
<td data-val="type">
...
<td data-val="name">
    <c:choose>
        <c:when test="${param.representativeName == null}">
            <c:out value="" />
        </c:when>
        <c:otherwise>
            <c:out value="${param.representativeName}" />
        </c:otherwise>
	</c:choose>
</td>

...
<c:set var="representative" value="${userPersonDetails.representatives[0]}" />
...
<dl class="column">
	...
	<dd>
		<c:if test="${not empty applicant.addresses}">
			<c:out value="${applicant.addresses[0].country}" />	
		</c:if>
	</dd>
	...
	<dd>
		<c:if test="${not empty applicant.addresses}">
			<c:out value="${applicant.addresses[0].street}" />
		</c:if>
	</dd>
</dl>
...
<dl class="column">
	...
	<dd><c:out value="${representative.legalForm}"></c:out></dd>
	
	<dt><spring:message code="review.person.user.details.type.title"/></dt>
	<dd><spring:message code="review.person.user.details.type.representative.${representative.representativeKind.name()}"/></dd>

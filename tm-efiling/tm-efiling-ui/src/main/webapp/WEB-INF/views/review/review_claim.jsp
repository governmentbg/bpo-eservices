		    ...
		    <table class="table">
		        ...
		        <c:forEach var="seniorities" items="${flowBean.seniorities}" varStatus="rowCounter">
		            <tr class="seniority">
						...
						<td class="name">
							<spring:eval var="storeprioritiesnumber" expression="seniorities.filingNumber" /> 
									<c:choose>
										<c:when test="${storeprioritiesnumber.length()>10}">
											${storeprioritiesnumber.substring(0,10)}...
										</c:when>
						...
						<td>
							<c:choose>
								<c:when
									test="${seniorities.fileWrapperCopy.attachment || seniorities.fileWrapperTranslation.attachment || seniorities.fileSeniorityCertificate.attachment}">
									<spring:message code="claim.table.documents.yes" />
									<c:forEach items="${seniorities.fileWrapperCopy.storedFiles}"
										var="file">
										<span class="file-size">${file.canonicalFileSize!=null
											&&
											file.canonicalFileSize.length()<=40?file.canonicalFileSize:file.canonicalFileSize.substring(0,40)}</span>
									</c:forEach>
									<c:forEach
										items="${seniorities.fileWrapperTranslation.storedFiles}"
										var="file">
										<span class="file-size">${file.canonicalFileSize!=null
											&&
											file.canonicalFileSize.length()<=40?file.canonicalFileSize:file.canonicalFileSize.substring(0,40)}</span>
									...
									<c:forEach
										items="${seniorities.fileSeniorityCertificate.storedFiles}"
										var="file">
										<span class="file-size">${file.canonicalFileSize!=null
											&&
											file.canonicalFileSize.length()<=40?file.canonicalFileSize:file.canonicalFileSize.substring(0,40)}</span>
					...
					<c:choose>
						<c:when test="${!(transformations.transformationCountryCode eq ctmTransformationOffice)}">
							<tr class="transformation">
								...
								<td>${transformations.irNumber}</td>
								<td></td>
							</tr>
						</c:when>
						<c:when test="${transformations.transformationCountryCode eq ctmTransformationOffice}">
							<tr class="ctmtransformation">
								<td class="number"><fmt:formatNumber value="${rowCounter.count}" minIntegerDigits="2" /></td>
								<td><spring:message code="claim.title.ctm.transformation"/></td>
								<td>-</td>
								<spring:eval var="dateFormat" expression="@propertyConfigurer.getProperty('iu.template.dateFormat')" />
								<td><fmt:formatDate type="date" pattern="${dateFormat}" value="${transformations.irDate}"/></td>
								<td>${transformations.irNumber}</td>
		        ...
		        <c:forEach var="conversions" items="${flowBean.conversions}" varStatus="rowCounter">
		            <tr class=conversions>
		                <td class="number"><fmt:formatNumber value="${rowCounter.count}" minIntegerDigits="2" /></td>
		                <td><spring:message code="claim.title.conversion"/></td>
		                <td>-</td>
		                <spring:eval var="dateFormat" expression="@propertyConfigurer.getProperty('iu.template.dateFormat')" />
		                <td><fmt:formatDate type="date" pattern="${dateFormat}" value="${conversions.conversionApplicationDate}"/></td>
		                <td>
		                <spring:eval var="storeprioritiesnumber" expression="conversions.conversionApplicationNumber"/>
									<c:choose>
										<c:when test="${storeprioritiesnumber.length()>10}">
											${storeprioritiesnumber.substring(0,10)}...
										</c:when>

...
<ul class="aside-options">
	...
	/><c:if test="${service_save_load && (security_save_load_locally || !configurationServiceDelegator.securityEnabled)}"
		><li>
			<div class="aside-option-load fileinput-button loadButton">
				<spring:message code="layout.applicationOptions.load"/>
				<input type="file" style="font-size:10000px" name="loadApplicationXML" class="submittedAjax uploadLoadApplication" data-url="loadApplication.htm?fileWrapperPath=${fileWrapperPath}&_csrf=${_csrf.token}&flowKey=${empty param.execution?param.flowKey:param.execution}">
			</div>
		</li></c:if
	...
	><li>
		<a class="aside-option-reset resetButton" data-url="${pageContext.request.contextPath}/${flowModeId}.htm"><spring:message code="layout.applicationOptions.reset" /></a>
	</li>
	<li>

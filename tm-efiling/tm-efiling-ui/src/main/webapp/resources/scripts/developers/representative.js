    ...
    displayChooseRepresentative: function()
    {
        ...
        $.ajax({
            ...
            success:function (html)
            {
                ...
                updateRepresentativeTypeIfOnlyOne();
...
{
	if ($("#legalForm").length>0 && $("#legalForm").val() == 'Natural Person') { 		
		document.getElementById('legalEntity').style.display = 'none';
	    document.getElementById('naturalPerson').style.display= 'block'; 
	} else if($("#legalForm").length>0 && $("#legalForm").val() == 'Legal Person') {
		document.getElementById('legalEntity').style.display = 'block';
    ...
    if ($("#representativeSection #importedRepresentative").val() == "true")
    {
        ...
        $("#legalForm").attr("disabled", "disabled");
    	$('#legalStatus').append('<input type="hidden" name="legalForm" value="'
    			+$("#legalForm").val()+'" />');
    }

    if ($("#reptype").val() == "PROFESSIONAL_PRACTITIONER") {
    	$("#legalForm").val("Natural Person");
    	$("#legalForm").attr("disabled", "disabled");
    	$('#legalStatus').append('<input type="hidden" name="legalForm" value="'
    			+$("#legalForm").val()+'" />');
    	document.getElementById('naturalPerson').style.display='block';
...
function updateRepresentativeTypeIfOnlyOne() {
    var enabled = $("#reptype > :enabled[value!='']");
    if (enabled.size() == 1) {
        fspLog("There is only one active representative type ..." + enabled.first().val() + "... Selecting it...");
        enabled.first().prop("selected", true);
        $("#reptype").change();
    }
}

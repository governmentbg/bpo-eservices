    ...
    $(this).data("autocomplete")._renderItem = function(ul, item)
    {
        ...
        if(searchUrlEnabled == "true")
        {
            ...
            wrappedUrl = "<span class='navigation-col' data-url='" +
                getApplicantAutocompleteNavigationUrl([["applicantId", item.aid]]) + "'>" +
...
function postUpdateApplicantSection()
{
    ...
    if($("#applicantSection input[name='nationalIdType']").length >0 && $("#applicantSection input[name='nationalIdType']:checked").length == 0){
        $($("#applicantSection input[name='nationalIdType']")[0]).prop("checked", true);
    }
...
function addCreateNewApplicantOnChangeFunctionality() {
    ...
    $("#applicantDetailsForm input:not(.editableImport):visible,#applicantDetailsForm textarea:visible,#applicantDetailsForm select:visible").each(function() {
        var el = $(this);
        el.data("prev",el.val());
        el.change(function() {
            if ($('#importedApplicant').val() == 'true') {

...
{
	var format = $(this).attr("data-id");
	var value = encodeURIComponent($(this).closest('tr').find('.value').html());
    ...
    $.ajax(
        {
            ...
            success:function (html)
            {
                ...
                // Check if there is any error, clear fields if not
                if($("#colourSection_internal .flMessageError").size() == 0){
                    $("#value").val("");
                    $(".editvalue").val("");
                    $(".editformat").val("");
                }

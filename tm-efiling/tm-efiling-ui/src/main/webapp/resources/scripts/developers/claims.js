function disableNoButtons()
{
    ...
    if ($("#claimTableContainer tr.ctmtransformation").length > 0)
    {
        $('#noCtmTransformation').bind('click', false);
        $('a#noCtmTransformation').addClass('disabled');
    } else
    {
        $('#noCtmTransformation').unbind('click', false);
        $('a#noCtmTransformation').removeClass('disabled');
    }
}
...
$('.claimSections .toggleNav .btn').live("click", function() {
    ...
    } else if($(this).attr('id') == 'noConversion'){
...
function markClaimNoButtonActive(rowsClass, noBtnSelector){
    var rows = $("#claimTableContainer table tr"+rowsClass).length;
    if(rows == 0){
        $(noBtnSelector).closest("li").addClass("active");
        $(noBtnSelector).closest("li").siblings("li").removeClass("active");
    }
}
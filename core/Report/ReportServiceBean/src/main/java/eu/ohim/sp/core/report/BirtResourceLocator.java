/*******************************************************************************
 * * $Id:: BirtResourceLocator.java 49261 2012-10-29 13:07:33Z karalch        $
 * *       . * .
...
/**
 * Sets the external resources path used by Birt Engine to generate the reports
 * Based on the Birt class "BIRTResourceLocator"
 * 
 * @author soriama
 *
 */
public class BirtResourceLocator implements IResourceLocator {
	...
	private final Logger logger = Logger.getLogger(BirtResourceLocator.class);
	...
	private URL getMessageFile(ModuleHandle moduleHandle, String fileName, int location) {
		...
		List<String> possibleFiles = BundleHelper.getHelper(moduleHandle.getModule(), fileName).getMessageFilenames(
				locale);

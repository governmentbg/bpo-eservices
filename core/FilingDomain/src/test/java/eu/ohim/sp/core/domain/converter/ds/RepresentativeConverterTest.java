...
public class RepresentativeConverterTest
{
    ...
    public void testConvertToFSP()
    {
        ...
        core.setReference("some reference");
        ...
        core.getPersonRoleRelationships().add(new PersonRoleRelationship());
        core.getPersonRoleRelationships().get(0).setDocuments(new ArrayList<AttachedDocument>());
        core.getPersonRoleRelationships().get(0).getDocuments().add(new AttachedDocument());
        core.getPersonRoleRelationships().get(0).getDocuments().get(0).setDocument(new Document());
        core.getPersonRoleRelationships().get(0).getDocuments().get(0).getDocument().setDocumentId("some document id");

        core.setIdentifiers(new ArrayList<PersonIdentifier>());
        PersonIdentifier afimi = new PersonIdentifier();
        afimi.setIdentifierKindCode("Afimi");
        afimi.setValue("afimi value");

        PersonIdentifier doy = new PersonIdentifier();
        doy.setIdentifierKindCode("DOY");
        doy.setValue("doy value");

        core.getIdentifiers().add(afimi);
        core.getIdentifiers().add(doy);

        PersonIdentifier nationalOfficialBusinessRegister = new PersonIdentifier();
        nationalOfficialBusinessRegister.setIdentifierKindCode("Company Number");
        nationalOfficialBusinessRegister.setValue("nobr");

        PersonIdentifier vatNumber = new PersonIdentifier();
        vatNumber.setIdentifierKindCode("VAT Number");
        vatNumber.setValue("vat");

        core.getIdentifiers().add(nationalOfficialBusinessRegister);
        core.getIdentifiers().add(vatNumber);
        
        core.setPrivacyWaiver(Boolean.TRUE);
        
        eu.ohim.sp.filing.domain.ds.Representative ext = dozerBeanMapper.map(core, eu.ohim.sp.filing.domain.ds.Representative.class);

        /// Assert
        assertEquals("some form", ext.getLegalForm());
        assertEquals("some reference", ext.getReference());
        ...
        assertEquals("some document id", ext.getRepresentativeDocumentDetails().getDocumentIncluded().get(0).getDocumentId());
        
        assertEquals(Boolean.TRUE, ext.getAddressBook().isDataPrivacyWaiver());

        assertEquals("Company Number", ext.getRepresentativeIdentifier().get(2).getIdentifierKindCode());
        assertEquals("nobr", ext.getRepresentativeIdentifier().get(2).getValue());

        assertEquals("VAT Number", ext.getRepresentativeIdentifier().get(3).getIdentifierKindCode());
        assertEquals("vat", ext.getRepresentativeIdentifier().get(3).getValue());

        assertEquals("afimi value", ext.getRepresentativeIdentifier().get(0).getValue());
        assertEquals("Afimi", ext.getRepresentativeIdentifier().get(0).getIdentifierKindCode());

        assertEquals("doy value", ext.getRepresentativeIdentifier().get(1).getValue());
        assertEquals("DOY", ext.getRepresentativeIdentifier().get(1).getIdentifierKindCode());
    }

    @Test
    public void testConvertToCore()
    {
        eu.ohim.sp.filing.domain.ds.Representative ext = new eu.ohim.sp.filing.domain.ds.Representative();
        ...
        ext.setRepresentativeIdentifier(new ArrayList<Identifier>());
        Identifier nobr = new Identifier("nobr", "Company Number");
        Identifier vat = new Identifier("vat", "VAT Number");
        ext.getRepresentativeIdentifier().add(nobr);
        ext.getRepresentativeIdentifier().add(vat);

        
        Representative core = dozerBeanMapper.map(ext, Representative.class);
        ...
        assertEquals("Company Number", core.getIdentifiers().get(0).getIdentifierKindCode());
        assertEquals("nobr", core.getIdentifiers().get(0).getValue());
        assertEquals("VAT Number", core.getIdentifiers().get(1).getIdentifierKindCode());
        assertEquals("vat", core.getIdentifiers().get(1).getValue());
    }

...
public class TradeMarkApplicationConverterTest {
	...
	public void testConvertFromClassOfTradeMarkApplicationObject() {
		...
		personRole.setAddresses(new ArrayList<eu.ohim.sp.core.domain.contact.Address>());
		...
		eu.ohim.sp.filing.domain.tm.TradeMarkApplication dest = dozerBeanMapper.map(
				tradeMarkApplication,
				eu.ohim.sp.filing.domain.tm.TradeMarkApplication.class);
        ...
        try {
            ...
            transaction.getTradeMarkTransactionBody()
        ...
        System.out.println(tradeMarkApplication.getTradeMark().getApplicationNumber());
        System.out.println(tradeMarkApplication.getTradeMark().getApplicationNumber());
		...
		assertEquals(tradeMarkApplication.getPayments().get(0)
				.getBankTransfer().getBankTransferDate(), dest
				.getPaymentDetails().getPayment().get(0).getPaymentMethod()
				.getBankTransfer().getBankTransferDate());

	}

	@Test
	public void testConvertToClassOfTradeMarkApplicationObject() {

		eu.ohim.sp.filing.domain.tm.TradeMarkApplication tradeMarkApplication = new eu.ohim.sp.filing.domain.tm.TradeMarkApplication();
        ...
        signatoryDetails.getSignatory().get(0).setCapacityText("text");
        signatoryDetails.getSignatory().get(0).setAssociatedText("associatedText");
        signatoryDetails.getSignatory().get(0).setEmail("email@sth.com");

        signatoryDetails.getSignatory().get(0).setConfirmation(Boolean.TRUE);
		...
		signatoryDetails.getSignatory().get(0).getName().getFormattedName()
		...
		tradeMarkApplication.setSignatoryDetails(signatoryDetails);

		tradeMarkApplication.setPaymentDetails(new PaymentDetails());
		tradeMarkApplication.getPaymentDetails().setPayment(new ArrayList<Payment>());
		tradeMarkApplication.getPaymentDetails().getPayment().add(new Payment());

		PaymentMethod paymentMethod = new PaymentMethod();
		paymentMethod.setBankTransfer(new eu.ohim.sp.filing.domain.tm.BankTransfer());
		paymentMethod.getBankTransfer().setBankDestinationAccount("bank destination");
		paymentMethod.getBankTransfer().setBankTransferDate(new Date());
		paymentMethod.getBankTransfer().setBankTransferIdentifier("bank transfer identifier");
		paymentMethod.getBankTransfer().setOriginBankName("origin bank name");

		tradeMarkApplication.getPaymentDetails().getPayment().get(0).setPaymentMethod(paymentMethod);
		...
		TradeMarkApplication dest = dozerBeanMapper.map(tradeMarkApplication,
				TradeMarkApplication.class);
		...
		assertEquals(dest.getTradeMark().getApplicationNumber(),
				tradeMarkApplication.getTradeMarkDetails().getTradeMark()
						.get(0).getApplicationNumber());
		assertEquals(dest.getTradeMark().getApplicants().get(0).getName()
				.getFirstName(), tradeMarkApplication.getTradeMarkDetails()
				.getTradeMark().get(0).getApplicants().getApplicant().get(0)
				.getAddressBook().getFormattedNameAddress().getName()
				.getFormattedName().getFirstName());
		assertEquals(dest.getTradeMark().getApplicants().get(0).getEmails()
				.get(0).getEmailAddress(), tradeMarkApplication
				.getTradeMarkDetails().getTradeMark().get(0).getApplicants()
				.getApplicant().get(0).getAddressBook()
				.getContactInformationDetails().getEmail().get(0));
		assertEquals(dest.getTradeMark().getRepresentatives().get(0)
				.getPhones().get(0).getNumber(), tradeMarkApplication
				.getTradeMarkDetails().getTradeMark().get(0)
				.getRepresentatives().getRepresentative().get(0)
				.getAddressBook().getContactInformationDetails().getPhone()
				.get(0).getValue());
		assertEquals(dest.getTradeMark().getExhibitionPriorities().get(0)
				.getExhibition().getCity(), tradeMarkApplication
				.getTradeMarkDetails().getTradeMark().get(0)
				.getExhibitionPriorities().getExhibitionPriority().get(0)
				.getCity());
		assertEquals(dest.getPayments().get(0)
				.getBankTransfer().getBankTransferDate(), tradeMarkApplication
				.getPaymentDetails().getPayment().get(0).getPaymentMethod()
				.getBankTransfer().getBankTransferDate());
	...
	private eu.ohim.sp.filing.domain.tm.TradeMark fillFspTradeMark() {
		...
		priorityDetails.setPriority(new ArrayList<eu.ohim.sp.filing.domain.tm.Priority>());
		...
		wordSpecification.setMarkVerbalElementText(new eu.ohim.sp.filing.domain.tm.Text());
		...
		signatory.setPlace("here");
        signatory.setCapacityText("capacityText");
        signatory.setAssociatedText("associatedText");
		...
		colourType.setColourClaimedText(new ArrayList<eu.ohim.sp.filing.domain.tm.Text>());
		...
		representative.getCorrespondenceAddresses().get(0)
				.getFormattedNameAddress().setName(new eu.ohim.sp.filing.domain.tm.Name());
		representative.getCorrespondenceAddresses().get(0)
		...
		representative.getAddressBook().getFormattedNameAddress().getName()
				.setFormattedName(new FormattedName());
		representative.getAddressBook().getFormattedNameAddress().getName()
				.getFormattedName().setFirstName("Georgios");
		representative.getAddressBook().getFormattedNameAddress().getName()
				.getFormattedName().setMiddleName("T");
		representative.getAddressBook().getFormattedNameAddress().getName()
				.getFormattedName().setLastName("Papadopoulos");

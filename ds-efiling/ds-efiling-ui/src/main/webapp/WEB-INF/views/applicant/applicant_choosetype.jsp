...
<c:set var="showApplicantUniversity" value="false"/>
<c:if test="${not empty configurationServiceDelegator.getApplicantTypes(flowModeId)}">
    <c:forEach var="section_item" items="${configurationServiceDelegator.getApplicantTypes(flowModeId)}">
		...
		<c:if test="${section_item eq 'applicant_university'}">
            <c:set var="showApplicantUniversity" value="false"/>
        </c:if>        
    </c:forEach>
</c:if>
...
<component:generic path="type" checkRender="true">
    ...
    <select name="aptype" id="aptype">
        ...
        <c:if test="${showApplicantUniversity}">
            <option value="UNIVERSITY"><spring:message code="applicant.university.type"/></option>
        </c:if>                 
    </select>
</component:generic>
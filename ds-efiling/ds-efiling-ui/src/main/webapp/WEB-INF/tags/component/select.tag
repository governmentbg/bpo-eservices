...
<jsp:directive.attribute name="multiple" required="false" type="java.lang.Boolean"
						 description="To render the component like a list box" />                         
...
<tags:render flowModeId="${flowModeId}" sectionId="${sectionId}" field="${path}" checkRender="${checkRender}">
    ...
    <c:if test="${fieldIsFastTrack}">
        <c:if test="${empty fastTrackValues}">
    ...
    <c:if test="${not empty labelTextCode}">
        <label class="${labelClass}"><spring:message code="${labelTextCode}"/>
    ...
    <c:set var="selectMultiple" value="" />
    <c:if test="${not empty multiple and multiple}">
    	<c:set var="selectMultiple" value="multiple" />
    </c:if>
    
    <form:select path="${path}" id="${id}" disabled="${!editableImport && not empty imported && imported}" cssClass="${formTagClass}" multiple="${selectMultiple}">
        <c:if test="${containsEmptyValue}">
            <spring:message var="emptyValueLabel" code="${emptyValueLabelCode}"/>
            <c:if test="${fieldIsFastTrack and (not empty fastTrackValues and fastTrackValues.contains(''))}">
                <c:set var="emptyValueLabel" value="${emptyValueLabel} &#128498;"/>
            </c:if>
            <form:option value="">${emptyValueLabel}</form:option>
        </c:if>
        ...
        <c:if test="${!empty items}">
            <c:forEach items="${items}" var="item">
                <c:if test="${empty itemFilter || item[itemFilter]}">
                    ...
                    <spring:message var="iitemLabel" code="${item[itemLabel]}"/>
                    <c:if test="${fieldIsFastTrack and (not empty fastTrackValues and fastTrackValues.contains(item[itemValue]))}">
                        <c:set var="iitemLabel" value="${iitemLabel} &#128498;"/>
                        <c:set var="itemClass" value="fasttrack"/>
                    </c:if>
                    ...
                    <c:if test="${(empty valueText || !valueText)&&empty textSelectedOption}">
                        <c:if test="${not empty dataaux}">
                            <form:option value="${item[itemValue]}" data-aux="${item[dataaux]}" selected="${itemSelected}" cssClass="${itemClass}">${iitemLabel}</form:option>
                        </c:if>
                        <c:if test="${empty dataaux}">
                            <form:option value="${item[itemValue]}" selected="${itemSelected}" cssClass="${itemClass}">${iitemLabel}</form:option>
                        </c:if>
                    </c:if>
                    ...
                    <c:if test="${not empty textSelectedOption && textSelectedOption}">
                        ...
                        <c:if test="${not empty dataaux}">
                            <form:option value="${item[itemValue]}" data-aux="${item[dataaux]}" cssClass="${itemClass}">${iitemLabel}</form:option>
                        </c:if>
                        <c:if test="${empty dataaux}">
                            <form:option value="${item[itemValue]}" cssClass="${itemClass}">${iitemLabel}</form:option>
                        </c:if>
                    </c:if>
                </c:if>
            </c:forEach>

...
<jsp:directive.attribute name="autocompleteServiceEnabled" required="false" type="java.lang.String" description="Boolean that says if the autocomplete functionality enabled"/>
...
<jsp:directive.attribute name="permissionAutocomplete" required="true" type="java.lang.Boolean" description="check for the permission of the functionality of autocomplete" />
...
<jsp:directive.attribute name="importTextfieldClass" required="false" type="java.lang.String" description="String specifying the import textbox's class"/>
...
<jsp:directive.attribute name="importButtonTextCode" required="false" type="java.lang.String" description="Code for import button for spring:message"/>
<jsp:directive.attribute name="component" required="false" type="java.lang.String" description="component for which we import"/>
<jsp:directive.attribute name="autocompleteClass" required="false" type="java.lang.String" description="when the component uses autocomplete, this class is provided as a css class to identify it"/>
<jsp:directive.attribute name="footerTextCode" required="false" type="java.lang.String" description="the text code of the footer"/>

<c:if test="${permissionAutocomplete and empty autocompleteServiceEnabled?true:configurationServiceDelegator.isServiceEnabled(autocompleteServiceEnabled)}">
    <c:set var="importInputClass" value="autocompleted ${importTextfieldClass}"/>
</c:if>
<c:if test="${not empty labelTextCode}">
    <label for="${id}"> <spring:message code="${labelTextCode}"/></label>
</c:if>
<c:if test="${not empty textCodeWhenEmpty}">
    <c:set var="textWhenEmpty"><spring:message code="${textCodeWhenEmpty}"/></c:set>
</c:if>
<c:if test="${not empty minimumCharAutocomplete}">
    <c:set var="minAutoComplete"><spring:message code="${minimumCharAutocomplete}"/></c:set>
</c:if>
<span class="autocomplete-input-holder">
    <span class="loading-small autocomplete-loading hidden"></span>
    <input type="text" name="${inputTextName}" size="100" maxlength="100" placeholder="${textWhenEmpty}" class="${importInputClass}" id="${id}" data-minchars="${minAutoComplete}" data-url="${autocompleteUrl}">
</span>
<button type="button" data-ask="${empty dataask?false:configurationServiceDelegator.getValue(dataask, component)}" id="${importButtonId}" class="${importButtonClass}">

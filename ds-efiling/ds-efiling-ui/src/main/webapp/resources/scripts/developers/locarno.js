...
!function() {
	...
	this.init = function() {
		...
		$(document).on('click', 'a[href = "#section_locarno"][data-target = "addClass"]', function() {
        	if (ds.efiling.application.isOneForm()) {
        		ds.efiling.application.hasSelectedLanguageWithCallback(this, ajax.loadAddClass);
        	} else {	
        		ajax.loadAddClass();
        	}
		});
		...
		$(document).on('click', 'a[href = "#section_locarno"][data-target = "addNewProduct"]', function() {
        	if (ds.efiling.application.isOneForm()) {
        		ds.efiling.application.hasSelectedLanguageWithCallback(this, ajax.loadAddNewProduct);
        	} else {	
		...
		$(document).on('click', '#locarnoList a[id ^= "iconEditNewProductAdded"]', function() {
			...
			if (ds.efiling.application.isOneForm()) {
        		ds.efiling.application.hasSelectedLanguageWithCallback(this, ajax.getNewProductForEdit, locarnoId);
        	} else {	
		...
		$(document).on('click', 'a[href = "#section_locarno"][data-target = "addNewComplexProduct"]', function() {
			if (ds.efiling.application.isOneForm()) {
        		ds.efiling.application.hasSelectedLanguageWithCallback(this, ajax.loadAddNewComplexProduct);
        	} else {	
        ...
        $(document).on('keypress', 'input[name="searchData.indication"]', function (event) {
            if(event.which == '13'){
        ...
        $(document).on('keypress', 'input[name="locarnoClassification.indication"]', function (event) {
            if(event.which == '13'){
                event.preventDefault();
                ajax.addNewProduct();
            }
	...
	var nav = {
		...
		integration: {
			...
			getClassifications: 'getLocarnoCassifications.htm'
		}
	};
	...
	var ajax = {
		...
		addNewComplexProduct: function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: $('#formAddNewComplexProduct').serialize(),
				success: function(html) {
					// We check if the controller has returned the same view, this will mean there are errors

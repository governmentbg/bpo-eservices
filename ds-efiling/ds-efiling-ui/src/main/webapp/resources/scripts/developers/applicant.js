...
Applicant.ajax =
{
    ...
    addApplicantPost: function(sender, address, ignoreMatches) {
        ...
        var designerIndicator = $("#designerIndicator").length > 0 && $("#designerIndicator").is(":checked");
        removeApplicantErrors();
        $.ajax(
            {
                url:address,
                ...
                success:function (html)
                {
                    ...
                    if (designerIndicator) {
                    	ds.efiling.application.oneForm.refreshDesignersTableWithAssociatedApplicant();
                    }
...
$(".addAppNaturalPerson").live("click", function (event, ignoreMatches)
{
    ...
    if ($("#designerIndicator").is(":checked")) {
    	...
    	showConfirmModal($("#applicant_addDesigner").html(), onConfirmYes);
    } else {

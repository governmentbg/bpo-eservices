...
<div id="main" role="main">
    <section class="done" id="basicPage">
        <div class="finished">
            ...
            <div class="info-details">
                ...
                <spring:eval expression="@propertyConfigurer.getProperty('appmanagement.application.url.pt')" var="appUrlPT"/>
                <spring:eval expression="@propertyConfigurer.getProperty('appmanagement.application.url.um')" var="appUrlUM"/>
                <spring:eval expression="@propertyConfigurer.getProperty('appmanagement.application.url.ep')" var="appUrlEP"/>

                <c:choose>
                    <c:when test="${submittedForm.flowModeId eq 'pt-efiling'}">
                        <c:set var="appUrl" value="${appUrlPT}"/>
                    </c:when>
                    <c:when test="${submittedForm.flowModeId eq 'um-efiling'}">
                        <c:set var="appUrl" value="${appUrlUM}"/>
                    </c:when>
                    <c:when test="${submittedForm.flowModeId eq 'ep-efiling'}">
                        <c:set var="appUrl" value="${appUrlEP}"/>
                    </c:when>
                </c:choose>
        ...
        <div class="info-submission">
            ...
            <ul>
                ...
                <li><spring:message code="info.submission.opt4"/></li>
            </ul>
        </div>
    </section>
	...
	<section class="follow-up">
		...
		<c:if test="${communication_inbox}">
			<a
				href="${configurationServiceDelegator.getValue('submitted.communication.url', 'ptefiling')}"
    ...
    <section class="warning">
        ...
        <c:if test="${misleading_information}">
            <a
                    href="${configurationServiceDelegator.getValue('submitted.misleading.url', 'ptefiling')}"
    ...
    <div class="wizard-steps-analytics" style="display:none;" data-ignore-parent="true">
        /patent/register-patent/submitted
    </div>
</div>
...
<sec:authorize var="authenticatedUser" access="isAuthenticated()" />
	...
	<header>
		...
		<c:if test="${not empty submittedForm}">
			<c:set var="flowModeId" value="${submittedForm.flowModeId}"/>
		</c:if>

		<a class="${env eq 'test' ? 'office-logo-test' : 'office-logo'}" href="./${flowModeId}.htm"></a>

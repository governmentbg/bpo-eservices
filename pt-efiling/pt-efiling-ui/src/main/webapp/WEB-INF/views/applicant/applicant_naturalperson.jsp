...
<form:form cssClass="formEF" id="applicantDetailsForm" modelAttribute="applicantNaturalPersonForm">
    ...
    <c:choose>
        <c:when test="${flowBean.earlierAppImported}">
           <input type="hidden" name="designerIndicator" value="false"/>
        </c:when>
        <c:otherwise>
            <component:checkbox path="designerIndicator" checkRender="true" id="designerIndicator"
                                labelTextCode="applicant.naturalPerson.field.${flowModeId == 'sv-efiling' ? 'authorIndicator' : 'inventorIndicator'}"/>
        </c:otherwise>
    </c:choose>

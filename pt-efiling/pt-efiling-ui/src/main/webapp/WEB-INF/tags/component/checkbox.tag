...
<jsp:directive.attribute name="disabled" required="false" type="java.lang.Boolean"
                         description="Indicates whether the component has to be disabled"/>
...
<tags:render flowModeId="${flowModeId}" sectionId="${sectionId}" field="${path}" checkRender="${checkRender}">
    ...
    <c:if test="${not empty labelTextCode}">
        <label class="${labelClass}" for="${id}">
            <form:checkbox path="${path}" id="${id}" cssClass="${formTagClass}" disabled="${!fieldIsEditable or disabled}"/>
    ...
    <c:if test="${empty labelTextCode}">
        <form:checkbox path="${path}" id="${id}" disabled="${!fieldIsEditable or disabled}"

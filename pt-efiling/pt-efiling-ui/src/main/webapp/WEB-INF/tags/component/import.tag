...
<c:choose>
    <c:when test="${permissionAutocomplete && (empty autocompleteServiceEnabled?true:configurationServiceDelegator.isServiceEnabled(autocompleteServiceEnabled))}">
        <c:set var="importInputClass" value="autocompleted ${importTextfieldClass}"/>
    </c:when>
    <c:otherwise>
        <c:set var="importInputClass" value="${importTextfieldClass}"/>
    </c:otherwise>
</c:choose>

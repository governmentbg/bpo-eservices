...
<jsp:directive.attribute name="tipCode" required="false" type="java.lang.String"
                         description="Code for the textarea tip"/>
...
<tags:render flowModeId="${flowModeId}" sectionId="${sectionId}" field="${path}" checkRender="${checkRender}">
    ...
    <form:textarea path="${path}" maxlength="4096" id="${id}" disabled="${!editableImport && not empty imported && imported}" cssClass="${formTagClass}"/>
    <c:if test="${not empty tipCode && (editableImport || empty imported || imported == false)}">
        <div class="tip"><spring:message code="${tipCode }" arguments="4096"/></div>
    </c:if>

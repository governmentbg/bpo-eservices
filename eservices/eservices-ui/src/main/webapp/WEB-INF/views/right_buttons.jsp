...
<ul class="aside-options">
	...
	<c:set var="service_save_load_locally" value="${configurationServiceDelegator.isServiceEnabled('Save_Load_Locally_Service')}" />
	<c:if test="${service_save_load_locally && (security_save_load_locally || !configurationServiceDelegator.securityEnabled)}">
		<li>
			<a class="aside-option-load fileinput-button loadButton">
				<spring:message code="layout.applicationOptions.load.${flowModeId}"/>
	...
	<c:if test="${service_save_load && (security_save_load_application || !configurationServiceDelegator.securityEnabled)}">
		<sec:authorize access="isAuthenticated()">
			<li>
				<a class="aside-option-save saveButton" href="#"><spring:message code="layout.applicationOptions.save.${flowModeId}" /></a>
			</li>
		</sec:authorize>
	</c:if>
	<c:if test="${service_save_load_locally && (security_save_load_locally || !configurationServiceDelegator.securityEnabled)}">
		<li>
			<a class="aside-option-save saveToPCButton" href="#"><spring:message code="layout.applicationOptions.saveToPC.${flowModeId}" /></a>
		</li>
	</c:if>
	<c:if test="${flowScopeDetails.stateId != 'review'}">
		<li>
			<a class="aside-option-reset resetButton" data-url="${pageContext.request.contextPath}/${flowModeId}.htm"><spring:message code="layout.applicationOptions.reset.${flowModeId}" /></a>
		</li>
	</c:if>
	<li>
